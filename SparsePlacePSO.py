...
class SparsePlaceRobust(SparsePlaceSpherical):
    ...
    def SparsePlacePSO(self, Jdes, Jsafety, focus_points, cancel_points, Af, Ac, beta, **kwargs):
        ...
        while n <= iters:
            ...
            def inv_objfn(conds):
                # save_cond = self.cond_vec.copy()
                print('\nHERE', conds.shape)
                res = []
                for condp in conds:
                    M = SparsePlaceSpherical(cond_vec=condp, **SPargs)
                    Afp, Acp = M.forward_model(focus_points, cancel_points)
                    objval = - (scale * np.sum(Acp@I_curr) + (- (Afp@I_curr) - Jdes))
                    print(objval, objval.shape)
                    res.append(objval[0])
                return np.asarray(res)

            totsize = self.cond_vec.size
            options = {'c1': 0.5, 'c2': 0.3, 'w':0.9, 'k': 2, 'p': 2}
            optimizer = ps.single.LocalBestPSO(n_particles=2, dimensions=totsize, options=options, bounds=bounds)
            val, optCond = optimizer.optimize(inv_objfn, iters=1)
