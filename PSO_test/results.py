...
# robust model on original A



# robust model on wc A
M = SparsePlaceSpherical(cond_vec=cond_vec, radius_vec=radius_vec, patch_size=patch_size,
    elec_radius=elec_radius, elec_spacing=elec_spacing, max_l=L, spacing=spacing,
    custom_grid=custom_grid, theta_elec=None, phi_elec=None)

Irob, aaxROBog = M(Af=Af_rob, Ac=Ac_rob, per=per, optimizer=1, Jdes=Jdes, 
    Jsafety=Jsafety, beta=beta, focus_points=focus_points, cancel_points=cancel_points, plot=False)

# rOg = np.sum(np.abs(Ac_rob@Iog))
# rRb = np.sum(np.abs(Ac_rob@Iwc))
