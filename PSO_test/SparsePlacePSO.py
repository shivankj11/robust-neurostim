...
class SparsePlaceSpherical(PreDefElecGrid, TransferFunction, CancelPointsSpherical):
    ...
    def plot_elec_pttrn(self, J, x_lim=None, y_lim=None, fname=None, disc_granularity=100):
        if self.custom_grid is False:
            elec_lst, ground_elec, elec_radius = self.uniform_sampling_north_pole(elec_spacing=self.elec_spacing * 10 ** (2), elec_radius=self.elec_radius * 10 ** (2), patch_size=self.patch_size * 10 ** (2))
        else:
            elec_lst, ground_elec, elec_radius = np.hstack((self.theta_elec.reshape(-1,1), self.phi_elec.reshape(-1,1))),  np.array((-np.pi/2.0,0),), self.elec_radius*np.ones((len(self.theta_elec)))*10**2
        ...
        x_discretize = np.arange(x_lim[0], x_lim[1] + self.eps, (x_lim[1] - x_lim[0]) / disc_granularity)
        y_discretize = np.arange(y_lim[0], y_lim[1] + self.eps, (y_lim[1] - y_lim[0]) / disc_granularity)

        spacing_x = (x_lim[1] - x_lim[0]) / disc_granularity
        spacing_y = (y_lim[1] - y_lim[0]) / disc_granularity
        data_projected = np.zeros((len(x_discretize), len(y_discretize)))
        for i in range(len(x_discretize)):
            print('Loading:', i, '/', disc_granularity, end='\r')
    ...
    def plot_given_curr_density(self, r, curr_density, x_limit=None, y_limit=None, abs=True, fname=None):
        ...
        plt.xticks(x_ticks, labels_x, fontsize='13')
        plt.yticks(y_ticks, labels_y, fontsize='13')
        plt.xlabel('x-axis (cm)', fontsize='17')
        plt.ylabel('y-axis (cm)', fontsize='17')
        plt.title('Current Density ('+str(round(self.r_max*10**2-r,2))+" cm)", fontsize='21')
        plt.tight_layout()
        if fname is not None:
            plt.savefig(fname)
...
class SparsePlaceRobust(SparsePlaceSpherical):

    def __init__(self, cond_vec, radius_vec, elec_radius=0.5, elec_spacing=None, patch_size=None, max_l=300,
    spacing=0.1, custom_grid=False, theta_elec=None, phi_elec=None, perturb=0.05, **kwargs):

        super().__init__(cond_vec, radius_vec, elec_radius, elec_spacing, patch_size, max_l, 
                         spacing, custom_grid, theta_elec, phi_elec)
        self.perturb = perturb
        self.kws = kwargs

    def SparsePlaceWorstCase_Hyperrectangle(self, Jdes, Jsafety, Af=None, Ac=None, beta=0.001, J_return=None, area_elec=None):
        """ min_x max_{perturbations at vetices of hyperrectangle} """
        if area_elec is None:
            area_elec = np.ones(Af.shape[1])
        I = cvx.Variable(Af.shape[1])

        condvec_og = self.cond_vec
        radvec_og = self.radius_vec
        ogvec = np.hstack(np.append(condvec_og, radvec_og))

        # self.cond_vec = condvec_og * self.perturb * (1 + np.array([random.choice((-1, 1)) for _ in range(len(self.cond_vec))]))
        # self.radius_vec = radvec_og * self.perturb * (1 + np.array([[random.choice((-1, 1)) for _ in range(len(self.radius_vec))]]))

        res = np.array([])
        vertex = self.perturb * np.hstack(np.array([-1, 1]))
        for v in itertools.product(vertex, repeat=len(ogvec)):
            newvec = v * ogvec
            self.cond_vec = np.asarray(newvec[:len(condvec_og)])
            self.radius_vec = np.asarray(newvec[len(condvec_og):])
    ...
    def dead_code():
        # condvec_og = self.cond_vec
            ...
            # # vertex = self.perturb * np.hstack(np.array([-1, 1]))
            # # vertices = np.array(list(itertools.product(vertex, repeat=len(ogvec))))
            # # newvec = vertices[0] * ogvec
            # self.perturb = 0
            # vertex = self.perturb * (np.array([random.choice((-1, 1)) for _ in range(len(ogvec))]))
            # print('vertex', vertex)
            # print(self.cond_vec * (1 + vertex[:len(condvec_og)]))
            # self.cond_vec *= (1 + vertex[:len(condvec_og)]) # np.asarray(newvec[:len(condvec_og)])
            # self.radius_vec *= (1 + vertex[len(condvec_og):]) # np.asarray(newvec[len(condvec_og):])
        pass

    def SparsePlaceWorstCase(self, Jdes, Jsafety, Af=None, Ac=None, beta=0.001, J_return=None, area_elec=None, getAs=False):
        """ get solution for I, then get worst case A and iterate until
            convergence for I solution or # iterations >= iters """
        
        I_val = super().SparsePlaceVanilla(Jdes, Jsafety, Af=Af, Ac=Ac, beta=beta)
        I_og = I_val.copy()

        if I_val is None:
            print('Initial problem infeasible')
            return
        
        print("Solved initial problem")

        conv_criteria = self.kws.get('conv_criteria', 0.01)
        tol = self.kws.get('tol', 0.001) # how much Af can vary from original matrix
        iters = self.kws.get('iters', 10)
        n = 1
        I_curr = I_val
        if area_elec is None:
            area_elec = np.ones(Af.shape[1])
        print(f'Starting {iters} iterations of min max')
        while n <= iters:

            Af_new = cvx.Variable(Af.shape)
            Ac_new = cvx.Variable(Ac.shape)
            constraints = [cvx.atoms.abs(Af - Af_new) <= tol * Af,
                           cvx.atoms.abs(Ac - Ac_new) <= tol * Ac]
            # obj = cvx.Maximize(cvx.atoms.abs(cvx.atoms.norm(Af_new@I_curr) - Jdes)) #+beta*cvx.atoms.norm1(I)
            foc_obj_term = -(cvx.atoms.norm(Af_new@I_curr) - Jdes)
            canc_obj_term = cvx.atoms.sum(Ac_new@I_curr)
            scale = Af.size / Ac.size
            scale = 0.2
            obj = cvx.Maximize(foc_obj_term + scale * canc_obj_term)
            prob = cvx.Problem(obj, constraints)
            ...
            # print(f'Success getting A (n={n})')
            # print(Af_new.value)
            # print(Af_new.value - Af)
            # print(Af_new.value @ I_val)
            # print(Af @ I_val)
            # print((cvx.atoms.norm(Af - Af_new)).value)
            # print((tol * cvx.atoms.norm(Af)).value)

            # print('\n\n\nAf-Anew:')
            # print(np.abs(Af - Af_new.value))
            # print('tol * Af:')
            # print(tol * Af, '\n\n\n')

            # minimizing (solving original problem for new A)
            I_curr = super().SparsePlaceVanilla(Jdes, Jsafety, Af=Af_new.value, Ac=Ac_new.value, beta=beta)
            # I_curr = super().SparsePlaceVanilla(Jdes, Jsafety, Af=Af_new.value, Ac=Ac, beta=beta)
            
            if I_curr is None:
                print('Failed getting I for new A matrix')
                return

            print(f'Success getting I (n={n})')

            I_diff = np.abs(I_val - I_curr)
            if (I_diff <= conv_criteria * np.abs(I_val)).all():
                # convergence condition is no significant change in I
                print('Convergence: Terminating early')
                break
        
            I_val = I_curr
            n += 1
        
        print("Original objective:", np.linalg.norm(Ac@I_og))
        print("Robust objective:", np.linalg.norm(Ac@I_val))
        if getAs:
            return I_curr, Af_new.value, Ac_new.value
        return I_curr
    
    def forward_model_params(self, focus_points, cancel_points):
    ...
    def SparsePlacePSOforA(self, Jdes, Jsafety, Af=None, Ac=None, beta=0.001, J_return=None, area_elec=None):
        """ solve PSO for I get solution for I, then get worst case A with PSO, iterate until
            convergence for I solution or # iterations >= iters """
        I_val = super().SparsePlaceVanilla(Jdes, Jsafety, Af=Af, Ac=Ac, beta=beta)
        I_og = I_val.copy()
        ...
        conv_criteria = self.kws.get('conv_criteria', 0.01)
        tol = self.kws.get('tol', 0.1) # how much Af can vary from original matrix
        iters = self.kws.get('iters', 10)
        n = 1
        I_curr = I_val
        print(f'Starting {iters} iterations of min max')
        while n <= iters:
            # running PSO to get A which maximizes objective
            Afbounds = (np.hstack(Af * (1 - tol)), np.hstack(Af * (1 + tol)))
            Acbounds = (np.hstack(Ac * (1 - tol)), np.hstack(Ac * (1 + tol)))
            bounds = (np.append(Afbounds[0], Acbounds[0]), np.append(Afbounds[1], Acbounds[1]))

            scale = 0.1
            def obj_fn(vals):
                objs = []
                for particle in vals:
                    Afvs = particle[:Af.size].reshape(Af.shape)
                    Acvs = particle[Af.size:].reshape(Ac.shape)
                    obj = -np.sum(Acvs) * scale - (Jdes - (Afvs @ I_curr))
                    objs.append(objs)
                return np.array(objs)

            totsize = Af.size + Ac.size
            options = {'c1': 0.5, 'c2': 0.3, 'w':0.9, 'k': 2, 'p': 2}
            optimizer = ps.single.LocalBestPSO(n_particles=10, dimensions=totsize, options=options, bounds=bounds)
            As = optimizer.optimize(obj_fn, iters=1)
            print(As)
            Af_new = As[:Af.size].reshape(Af.shape)
            Ac_new = As[Af.size:].reshape(Ac.shape)
    ...
    def SparsePlacePSO(self, Jdes, Jsafety, focus_points, cancel_points, Af, Ac, beta, **kwargs):
        """ solve SparsePlace to get solution for I, then get worst case A with PSO as
            fn of params, iterate until convergence for I solution or # iterations >= iters """
        SPargs = kwargs.get('SPargs')
        ...
        conv_criteria = self.kws.get('conv_criteria', 0.01)
        # how much conductivity values can vary from originals
        tol = self.kws.get('tol', 0.1)
        iters = self.kws.get('iters', 5)
        n = 1
        I_curr = I_val
        print(f'Starting {iters} iterations of min max')
        while n <= iters:
            ...
            # # obj = cvx.Maximize(cvx.atoms.abs(cvx.atoms.norm(Af_new@I_curr) - Jdes)) #+beta*cvx.atoms.norm1(I)
            # foc_obj_term = -(cvx.atoms.norm(Af_new@I_curr) - Jdes)
            # canc_obj_term = cvx.atoms.norm(Ac_new@I_curr)
            # obj = cvx.Maximize(cvx.sum(Ac_new@I_curr, Af_new@I_curr))
            
            # constraints = [cvx.atoms.abs(Af - Af_new) <= tol * Af,
            #                cvx.atoms.abs(Ac - Ac_new) <= tol * Ac]
            # obj = cvx.Maximize(cvx.atoms.abs(cvx.atoms.norm(Af_new@I_curr) - Jdes))
            # scale = 0.1 # ~ |Af| / |Ac|
            # obj: (- (Af @ I) - Jdes) + scale * sum(Ac @ I)
            
            # solve for Af, Ac using PSO to find optimal of obj wrt radius and conductivity
            # start w just conductivity
            # copy in forward model to calculate Af, Ac
            # constraints will be only the bounds on params

            '''
            set up conductivity params and bounds
            set up fn to get: f = (params -> Af, Ac)
            set up fn to get obj from f: obj = (params -> obj)
            run PSO and return Af, Ac as normal below
            '''
            ...
            # x_min = x_max
            scale = 0.1 # 
            bounds = (np.hstack(self.cond_vec * (1 - tol)), np.hstack(self.cond_vec * (1 + tol)))
            def inv_objfn(conds):
                i = 0
                res = []
                for condp in conds:
                    np.save("obj_update.npy", i)
                    i += 1
                    M = SparsePlaceSpherical(cond_vec=condp, **SPargs)
                    Afp, Acp = M.forward_model(focus_points, cancel_points)
                    objval = - (scale * np.sum(Acp@I_curr) + np.abs((Afp@I_curr) - Jdes))
                    res.append(objval[0])
                return np.asarray(res)

            totsize = self.cond_vec.size
            options = {'c1': 0.5, 'c2': 0.3, 'w':0.9, 'k': 2, 'p': 2}
            optimizer = ps.single.LocalBestPSO(n_particles=25, dimensions=totsize, options=options, bounds=bounds)
            val, optCond = optimizer.optimize(inv_objfn, iters=10)
            M = SparsePlaceSpherical(cond_vec=optCond, **SPargs)
            Af_new, Ac_new = M.forward_model(focus_points, cancel_points)
            
            # Af_new = As[:Af.size].reshape(Af.shape)
            # Ac_new = As[Af.size:].reshape(Ac.shape)
            np.save("working_Af2.npy", Af_new)
            np.save("working_Ac2.npy", Ac_new)
            print(f'Success getting A (n={n})')
            # print(Af_new.value)
            # print(Af_new.value - Af)
            # print(Af_new.value @ I_val)
            ...
            # minimizing (solving original problem for new A)
            I_curr = super().SparsePlaceVanilla(Jdes, Jsafety, Af=Af_new, Ac=Ac_new, beta=beta)
    ...
    def SparsePlaceVanilla(self, *args, **kwargs):
        return self.SparsePlaceWorstCase(*args, **kwargs)
