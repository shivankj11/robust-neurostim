...
class SparsePlaceRobust(SparsePlaceSpherical):
    ...
    spacing=0.1, custom_grid=False, theta_elec=None, phi_elec=None, perturb=0.05):
    ...
    def SparsePlaceWorstCase(self, Jdes, Jsafety, iters=10, Af=None, Ac=None, beta=0.001, J_return=None, area_elec=None):
        """ get solution for I, then get worst case A and iterate until
            convergence for I solution or # iterations >= iters """
        if area_elec is None:
            area_elec = np.ones(Af.shape[1])

        print(self.cond_vec, self.cond_vec.shape)
        print(self.radius_vec, self.radius_vec.shape)
        ...
        if J_return is None:
            ...
            ones = np.zeros((1, Af.shape[1]))
            ones[len(ones) - 1] = 1
            constraints = [Af @ I == Jdes,
                           ones@I==J_return, cvx.sum(I) == 0,
                           cvx.atoms.norm_inf(I) <= Jsafety,
                           cvx.atoms.norm1(cvx.multiply(I, area_elec)) <= beta]
        obj = cvx.Minimize(cvx.atoms.norm(Ac@I)) # +beta*cvx.atoms.norm1(I)
        prob = cvx.Problem(obj, constraints)
        prob.solve(verbose=True, solver=cvx.ECOS, feastol=1e-8)  # Returns the optimal value.
        ...
        conv_criteria = 0.1
        tol = 50 # how much Af can vary from original matrix
        n = 1
        I_curr = I_val
        ...
        while n <= iters:
            ...
            # maximizing
            A_new = cvx.Variable(Af.shape)
            constraints = [cvx.atoms.norm(Af - A_new) <= tol * cvx.atoms.norm(Af)]
            obj = cvx.Maximize(-cvx.atoms.norm(A_new@I_curr - Jdes)) #+beta*cvx.atoms.norm1(I)
            prob = cvx.Problem(obj, constraints)
            ...
            print(A_new.value)
            print(A_new.value - Af)
            print(A_new.value @ I_val)
            print(Af @ I_val)
            print((cvx.atoms.norm(Af - A_new)).value)
            ...
            I = cvx.Variable(Af.shape[1])
            if J_return is None:
                constraints = [A_new.value @ I == Jdes, cvx.sum(I) == 0, cvx.atoms.norm_inf(I)<=Jsafety, cvx.atoms.norm1(I)<=beta]
            else:
                ones = np.zeros((1, A_new.value.shape[1]))
                ones[len(ones) - 1] = 1
                constraints = [A_new.value @ I == Jdes,
                               ones@I==J_return, cvx.sum(I) == 0,
                               cvx.atoms.norm_inf(I) <= Jsafety,
                               cvx.atoms.norm1(cvx.multiply(I, area_elec)) <= beta]
            obj = cvx.Minimize(cvx.atoms.norm(Ac@I)) # +beta*cvx.atoms.norm1(I)
            ...
            print('\n\nIDIFF:')
            print(I_diff, '\n\n')
            if (I_diff <= conv_criteria * I_curr).all():
    ...
    def SparsePlaceVanilla(self, *args, **kwargs):
        # print('HERE', self.SparsePlaceWorstCase(*args, **kwargs))
